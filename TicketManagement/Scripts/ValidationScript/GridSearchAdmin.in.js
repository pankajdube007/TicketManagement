$(document).ready(function () {
    $('#GridTableContainer').jtable({
        ...
        actions:
        {
            listAction: '/AdminDashboard/GetAllTickets',
            deleteAction: '/AdminDashboard/DeleteTicket'
        },
    ...
    $("#Username").autocomplete({
        source: function (request, response) {
            $.ajax({
                url: "/AdminDashboard/GetAllAgentUsers",
                type: "GET",
                dataType: "json",
...
function Ticket(trackingId) {
    window.location.href = '/TicketDetailsAdmin/Details?TrackingId=' + trackingId;
    return false;
}
...
function processChangeAllPriority(values) {
    $.ajax({
        ...
        url: "/AdminDashboard/ProcessChangeAllPriority",
...
function ProcessChangeAllStatus(values) {
    $.ajax({
        type: "POST", //HTTP POST Method
        url: "/AdminDashboard/ProcessChangeAllStatus",
...
function AssignUserTicket() {
    ...
    if ($selectedRows.length > 0) {
        ...
        if (values != "") {
            $.ajax({
                type: "POST", //HTTP POST Method
                url: "/AdminDashboard/AssignTickettoUser",
...
var url = "/AdminDashboard/GetAllSearchFields";
...
var url = "/AdminDashboard/GetAllPriority";
$.getJSON(url, function (data) {
    if (data) {
        $("#DropPriority").empty();
...
var url = "/AdminDashboard/GetAllStatus";
$.getJSON(url, function (data) {
    if (data) {
        $("#DropStatus").empty();

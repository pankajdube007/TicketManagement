...
namespace TicketManagement.Helpers
{
    public class TicketHistoryHelper
    {
       // private readonly DatabaseContext _context;
        ...
        public string CreateMessage(long? priorityId, long? categoryId)
        {
        ...
        private string GetPriorityNameBypriorityId(long? priorityId)
        {
            var priorityList = (from priority in _context.Priorities
                                where priority.SLNO == priorityId
                                select priority.PriorityName).FirstOrDefault();
            return priorityList;
        }

        private string GetStatusBystatusId(long? statusId)
        {
            var priorityList = (from status in _context.Status
                                where status.SLNO == statusId
                                select status.StatusText).FirstOrDefault();
            return priorityList;
        }

        private string GetCategoryNameBycategoryId(long? categoryId)
        {
            var priorityList = (from category in _context.CategoryMasters

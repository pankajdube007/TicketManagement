    ...
    public class HODDashboardController : Controller
    {
        ...
        public ActionResult Dashboard()
        {
            try
            {
                ...
                var ticketcounts = _dashboardTicketCount.GetAllTicketbyStatusofCurrentDayforAgentAdmin(_sessionHandler.HodCategoryId);
        ...
        public ActionResult ShowMenus()
        {
            try
            {
                ...
                return PartialView("ShowMenuHOD", menuList);
            }
            catch (Exception)
            {
                throw;
        ...
        public JsonResult GetAllTickets(string search, string searchin, int? prioritysearch, int? statussearch, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                ...
                var categoryId = Convert.ToInt32(_sessionHandler.HodCategoryId);
        ...
        public JsonResult ShowBarCharts()
        {
            try
            {
                var listofdata = _chart.GetListofBarChartDataAgentsAdmin(Convert.ToInt32(_sessionHandler.HodCategoryId));
        ...
        public JsonResult GetAllDeletedTickets(string search, string searchin, int? prioritysearch, int? statussearch, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                ...
                var totalTicketsCount = _iTickets.GetDeleteTicketsAgentsAdminCount(currentUserId, _sessionHandler.HodCategoryId);
                var recordcount = _iTickets.GetAllAgentsAdminDeleteTickets(search, searchin, Convert.ToInt32(_sessionHandler.HodCategoryId), prioritysearch, statussearch, jtStartIndex, jtPageSize, jtSorting);
                return Json(new { Result = "OK", Records = recordcount, TotalRecordCount = totalTicketsCount });
            }
            catch (Exception)
            {
        ...
        {
            var userlist = _userMaster.GetActiveUsersbyCategoryId(usernames, _sessionHandler.HodCategoryId);
        ...
        [HttpPost]
        public JsonResult GetAllHodTickets(string search, string searchin, int? prioritysearch, int? statussearch, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {

                var categoryId = Convert.ToInt32(_sessionHandler.HodCategoryId);
                var totalTicketsCount = _allTicketGrid.AllHodTicketsCount(search, searchin, prioritysearch, statussearch, categoryId);
                var recordcount = _allTicketGrid.AllHodTicketsbyList(search, searchin, categoryId, prioritysearch, statussearch, jtStartIndex, jtPageSize, jtSorting);
                return Json(new { Result = "OK", Records = recordcount, TotalRecordCount = totalTicketsCount });
            }
            catch (Exception)
            {

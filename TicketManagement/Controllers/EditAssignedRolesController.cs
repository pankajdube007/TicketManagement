                ...
                if (!string.IsNullOrEmpty(username))
                {
                    using (var db = new DatabaseContext())
                    {
                        var data = (from usermaster in db.Usermasters
                                    join sar in db.SavedAssignedRoles on usermaster.SLNO equals sar.UserId
                                    join roleMaster in db.Role on sar.RoleId equals roleMaster.RoleId
                                    where usermaster.IsActive == true && usermaster.UserName == username
                                    select usermaster
                            ).Count();
                ...
                else
                {
                    using (var db = new DatabaseContext())
                    {
                        var data = (from usermaster in db.Usermasters
                                join sar in db.SavedAssignedRoles on usermaster.SLNO equals sar.UserId
                                join roleMaster in db.Role on sar.RoleId equals roleMaster.RoleId
                                where usermaster.IsActive == true 
                                select usermaster
                            ).Count();

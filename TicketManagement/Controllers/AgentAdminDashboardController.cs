...
namespace TicketManagement.Controllers
{
    ...
    public class AgentAdminDashboardController : Controller
    {
        // GET: AgentAdminDashboard
        ...
        public AgentAdminDashboardController(IMenu menu,
            ISubMenu subMenu,
            ITickets tickets,
            IPriority priority,
            IStatus status,
        ...
        public ActionResult Dashboard()
        {
            try
            {
                ...
                var ticketcounts = _dashboardTicketCount.GetAllTicketbyStatusofCurrentDayforAgentAdmin(_sessionHandler.AgentAdminCategoryId);
        ...
        public ActionResult ShowMenus()
        {
            try
            {
                ...
                return PartialView("ShowMenuAgentsAdmin", menuList);
            }
            catch (Exception)
            {
                throw;
        ...
        public JsonResult GetAllTickets(string search, string searchin, int? prioritysearch, int? statussearch, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                ...
                var categoryId = Convert.ToInt32(_sessionHandler.AgentAdminCategoryId);
                var totalTicketsCount = _iTickets.GetAllAgentAdminTicketsCount(search, searchin, prioritysearch, statussearch, categoryId);
                var recordcount = _iTickets.GetAllAgentAdminTicketsbyList(search, searchin, categoryId, prioritysearch, statussearch, jtStartIndex, jtPageSize, jtSorting);
        ...
        public JsonResult ShowBarCharts()
        {
            try
            {
                var listofdata = _chart.GetListofBarChartDataAgentsAdmin(Convert.ToInt32(_sessionHandler.AgentAdminCategoryId));
        ...
        public JsonResult ShowDonutCharts()
        {
            ...
            try
            {
                ...
                var listofdata = _chart.GetListofDonutChartDataAgentsAdmin(Convert.ToInt32(_sessionHandler.AgentAdminCategoryId));
            ...
            try
            {
                if (!string.IsNullOrEmpty(ticketid))
                {
                    ...
                    if (result > 0)
                    {
                        ...
                        TicketManagement.Reposistory.TicketHistory ticketHistory = new TicketHistory
                        {
        ...
        public JsonResult GetAllDeletedTickets(string search, string searchin, int? prioritysearch, int? statussearch, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                ...
                var totalTicketsCount = _iTickets.GetDeleteTicketsAgentsAdminCount(currentUserId, _sessionHandler.AgentAdminCategoryId);
                var recordcount = _iTickets.GetAllAgentsAdminDeleteTickets(search, searchin, Convert.ToInt32(_sessionHandler.AgentAdminCategoryId), prioritysearch, statussearch, jtStartIndex, jtPageSize, jtSorting);
                return Json(new { Result = "OK", Records = recordcount, TotalRecordCount = totalTicketsCount });
            }
            catch (Exception)
            {
        ...
        {
            var userlist = _userMaster.GetActiveUsersbyCategoryId(usernames, _sessionHandler.AgentAdminCategoryId);
        ...
        [HttpPost]
        public JsonResult GetAllAgentAdminTickets(string search, string searchin, int? prioritysearch, int? statussearch, int jtStartIndex = 0, int jtPageSize = 0, string jtSorting = null)
        {
            try
            {
                var categoryId = Convert.ToInt32(_sessionHandler.AgentAdminCategoryId);
                var totalTicketsCount = _allTicketGrid.AllAgentAdminTicketsCount(search, searchin, prioritysearch, statussearch, categoryId);
                var recordcount = _allTicketGrid.AllAgentAdminTicketsbyList(search, searchin, categoryId, prioritysearch, statussearch, jtStartIndex, jtPageSize, jtSorting);
                return Json(new { Result = "OK", Records = recordcount, TotalRecordCount = totalTicketsCount });
            }
            catch (Exception)
            {

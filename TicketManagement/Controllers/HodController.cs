...
namespace TicketManagement.Controllers
{
    ...
    public class HodController : Controller
    {
        ...
        public HodController(IProfile profile, IUserMaster userMaster, IPassword password)
        {
            _iProfile = profile;
        ...
        public ActionResult ProfileInformation()
        {
            try
            {
                var hodprofile = _iProfile.GetprofileById(Convert.ToInt64(_sessionHandler.UserId));
                return View(hodprofile);
            }
            catch (Exception)
            {
                throw;
        ...
        [HttpPost]
        public ActionResult ProfileInformation(UsermasterEditView usermasterEditView)
        {
            try
            {
        ...
        public ActionResult UpdateSignature(ViewSignatureRequestViewModel viewSignatureRequestViewModel)
        {
            try
            {
                if (!string.IsNullOrEmpty(viewSignatureRequestViewModel.Signature))
                {
                    ...
                    {
                        Models.Signatures signatures = new Signatures()
                        {
                            UserId = Convert.ToInt64(_sessionHandler.UserId),
                            Signature = viewSignatureRequestViewModel.Signature,
                            SignatureId = 0
                ...
                if (ModelState.IsValid)
                {
                   ...
                   // var generatehash = GenerateHashSha512.Sha512(changePasswordViewModel.ExistingPassword, usersalt.PasswordSalt);
                    ...
                    else
                    {
                        ...
                        return RedirectToAction("Changepassword", "Hod");
                    }
                }
                return View(changePasswordViewModel);
            }

...
namespace TicketManagement.Controllers
{
    ...
    public class AgentAdminController : Controller
    {
        ...
        public AgentAdminController(IProfile profile, IUserMaster userMaster, IPassword password)
        {
            _iProfile = profile;
        ...
        public ActionResult ProfileInformation()
        {
            try
            {
                var agentadminprofile = _iProfile.GetprofileById(Convert.ToInt64(_sessionHandler.UserId));
                return View(agentadminprofile);
            }
            catch (Exception)
            {
                throw;
                ...
                if (ModelState.IsValid)
                {
                  ...
                  //  var generatehash = GenerateHashSha512.Sha512(changePasswordViewModel.ExistingPassword, usersalt.PasswordSalt);
                    ...
                    else
                    {
                        ...
                        return RedirectToAction("Changepassword", "AgentAdmin");
                    }
                }
                return View(changePasswordViewModel);
            }

...
namespace TicketManagement.Controllers
{
    ...
    public class RoleMasterController : Controller
    {
        ...
        public ActionResult Create([Bind(Include = "RoleId,RoleName,Status")] Role roleMaster)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    R.RoleName = roleMaster.RoleName;
                    R.IsActive = roleMaster.Status;
                    R.CreateId= Convert.ToInt32(_sessionHandler.UserId);
                    R.LogNo = Convert.ToInt64(Response.Cookies["logno"].Value);
                    _iRole.AddRoleMaster(R);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "SLNO,RoleName,IsActive,Createdt")] RoleMaster roleMaster)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    roleMaster.RoleId = Convert.ToInt32(roleMaster.SLNO);
                    roleMaster.ModifyId=Convert.ToInt32(_sessionHandler.UserId);
        ...
        public ActionResult Delete(int? id)
        {
            ...
            //        return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
            //    }
            //    Role roleMaster = _iRole.GetRoleMasterById(id);
            //    if (roleMaster == null)
            //    {
            //        return HttpNotFound();
            //    }

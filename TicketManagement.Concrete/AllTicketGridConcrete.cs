    ...
    public class AllTicketGridConcrete : IAllTicketGrid
    {
        ...
        public List<ViewTicketModel> AllUserTickets(string search, string searchin, int? statusId, long userId, int startIndex, int count, string sorting)
        {
            ...
            var query = (from tickets in _context.Tickets
                         join ticketStatus in _context.TicketStatus on tickets.SlNo equals ticketStatus.TicketId
                             into ticketStatusgroup
                         from t in ticketStatusgroup.DefaultIfEmpty()

                         join category in _context.Category on tickets.CategoryId equals category.SLNO
                         join ticketdetails in _context.TicketDetails on tickets.SlNo equals ticketdetails
                             .TicketId
                         join status in _context.Status on t.StatusId equals status.SlNO into statusgroup
                         from s in statusgroup.DefaultIfEmpty()
                         join priority in _context.Priority on t.PriorityId equals priority.SLNO into
                             prioritygroup
                         from p in prioritygroup.DefaultIfEmpty()
                         join dl in _context.TicketDeleteLockStatus on tickets.SlNo equals dl.TicketId

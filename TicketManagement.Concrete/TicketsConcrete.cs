...
namespace TicketManagement.Concrete
{
    public class TicketsConcrete : ITickets
    {
            ...
            List<Attachments> attachments, List<AttachmentDetails> attachmentDetails)
        {
                ...
                new SqlConnection(ConfigurationManager.ConnectionStrings["DatabaseConnection"].ConnectionString))
            {
                ...
                try
                {
                    ...
                    paramticket.Add("@Logno", tickets.LogNo);
                    ...
                    paramticketDetails.Add("@Logno", tickets.LogNo);
                    ...
                    if (attachments != null)
                    {
                        for (var i = 0; i < attachments.Count; i++)
                        {
                            ...
                            paramAttachments.Add("@Logno", tickets.LogNo);
                            ...
                            paramAttachmentsdetail.Add("@Logno", tickets.LogNo);
        ...
        public List<ViewTicketModel> GetAllTicketsbyUserId(string search, string searchin, int? statusId, long userId, int startIndex, int count, string sorting)
        {
                ...
                var query = (from tickets in db.Tickets
                             join ticketStatus in db.TicketStatus on tickets.SLNO equals ticketStatus.TicketId
                                 into ticketStatusgroup
                             from t in ticketStatusgroup.DefaultIfEmpty()

                             join category in db.CategoryMasters on tickets.CategoryId equals category.SLNO
                             join ticketdetails in db.TicketDetails on tickets.SLNO equals ticketdetails
                                 .TicketId
                             join status in db.Status on t.StatusId equals status.SLNO into statusgroup
                             from s in statusgroup.DefaultIfEmpty()
                             join priority in db.Priorities on t.PriorityId equals priority.SLNO into
                                 prioritygroup
                             from p in prioritygroup.DefaultIfEmpty()
                             join dl in db.TicketDeleteLockStatus on tickets.SLNO equals dl.TicketId

                             select new ViewTicketModel()
                             {
                                 TicketId = tickets.SLNO,
                                 Name = tickets.Name,
            ...
            try
            {
                using (var db = new DatabaseContext())
                {
                    var query = _context.Tickets
                        ...
                        .Join(_context.Priority, t => t.t3.t2.ticketStatus.PriorityId, p => p.SLNO,
                        ...
                        .Select(x => new ViewTicketModel
                        {
                            ...
                            PriorityId = x.t6.t5.priority.SLNO,
            ...
            int? statussearch, int startIndex, int count, string sorting)
        {
            try
            {
                ...
                {
                    var query = _context.Tickets
                        .Join(_context.TicketStatus, t => t.SlNo, ts => ts.TicketId,
                            (t, ts) => new { t1 = t, ticketStatus = ts })

                        .Join(_context.TicketDetails, t => t.t1.SlNo, td => td.TicketId,
                            (t, td) => new { t2 = t, ticketDetails = td })

                        .Join(_context.Status, t => t.t2.ticketStatus.StatusId, st => st.SlNO,
                            (t, st) => new { t3 = t, status = st })

                        .Join(_context.Priority, t => t.t3.t2.ticketStatus.PriorityId, p => p.SLNO,
                            (t, p) => new { t4 = t, priority = p })
                        .Join(_context.Category, t => t.t4.t3.t2.t1.CategoryId, c => c.SLNO,
                            (t, c) => new { t5 = t, Category = c })

                        .Join(_context.TicketDeleteLockStatus, t => t.t5.t4.t3.t2.t1.SlNo, dl => dl.TicketId,
                            (t, dl) => new { t6 = t, deletelock = dl })

                        .Select(x => new ViewTicketModel
                        {
                            TicketId = x.t6.t5.t4.t3.t2.t1.SlNo,
                            Name = x.t6.t5.t4.t3.t2.t1.Name,
                            Category = x.t6.Category.CategoryName,
                            Subject = x.t6.t5.t4.t3.ticketDetails.Subject,
                            TrackingId = x.t6.t5.t4.t3.t2.t1.TrackingId,
                            Status = x.t6.t5.t4.status.StatusText,
                            Priority = x.t6.t5.priority.PriorityName,
                            StatusId = x.t6.t5.t4.status.SlNO,
                            PriorityId = x.t6.t5.priority.SLNO,
                            StatusAssigned = x.t6.t5.t4.t3.t2.t1.StatusAssigned,
                            UserId = x.t6.t5.t4.t3.t2.ticketStatus.UserId,
                            TicketUpdatedDate = x.t6.t5.t4.t3.t2.ticketStatus.TicketUpdatedDate,
